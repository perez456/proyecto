<?php
	...
	*/
	class Cita
	{
		...
		{
			$query  ="SELECT  cliente.direccionCli, cita.codigoCita, cita.documentoCli, cita.documentoEmp, cita.fecha,cita.hora, cita.estadoCita,cita.nombreServicio,cita.Servicio2,cita.Servicio3 FROM empleado INNER JOIN cita ON cita.documentoEmp = empleado.documentoEmp INNER JOIN cliente ON cita.documentoCli = cliente.documentoCli WHERE estadoCita = 'Asignada'";
		...
		public function setEditCita($data){
			if(!empty($data['codigoCita'])){
				$query  ="UPDATE cita SET documentoCli='".$data['documentoCli']."',documentoEmp='".$data['documentoEmp']."',fecha='".$data['fecha']."',hora='".$data['hora']."',estadoCita='".$estadoFinal."',nombreServicio='".$data['nombreServicio']."',Servicio2='".$data['Servicio2']."',Servicio3='".$data['Servicio3']."' WHERE codigoCita=".$data['codigoCita'];
				$result =mysqli_query($this->link,$query);
				if($result){
					return true;
				}else{
		...
		public function setEditCitaEmp($data){
			if(!empty($data['codigoCita'])){
				$estadoFinal="Finalizada";
				$query  ="UPDATE `cita` SET `estadoCita` = 'Finalizada' WHERE `cita`.`codigoCita` =".$data['codigoCita'];
				$result =mysqli_query($this->link,$query);
				if($result){
					return true;
				}else{
		...
		public function getCitaBySearch($data=NULL){ 
			if(!empty($data)){
				$query  ="SELECT cliente.direccionCli, cita.codigoCita, cita.documentoCli, cita.documentoEmp, cita.fecha,cita.hora, cita.estadoCita,cita.nombreServicio,cita.Servicio2,cita.Servicio3 FROM cliente INNER JOIN cita ON cita.documentoCli = cliente.documentoCli WHERE codigoCita ='".$data."'";
				$result =mysqli_query($this->link,$query);
				return $result;
			}else{
				return false;
